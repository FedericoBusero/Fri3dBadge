...
#include "Buzzer.h"
#include "Button.h"
#include "Fri3dConfig.h"

Fri3dAccelerometer accel = Fri3dAccelerometer();
Fri3dMatrix matrix = Fri3dMatrix();
Buzzer buzzer = Buzzer(BUZZER_PIN, 0);
Button button0    = Button(BUTTON0_PIN);
Button button1    = Button(BUTTON1_PIN);
...
void setup() {
  ...
  button0.setReleasedCallback( nextState );
  button1.setReleasedCallback( nextState );
...
void loop() {
  button0.debounceButton(  );
  button1.debounceButton(  );

  if ( button0.getDebouncedState() )
    renderPixels();
  else
    switch (state) {
      case STATE_EYES:
